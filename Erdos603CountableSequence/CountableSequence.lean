...
/-!
# Erdős Problem 603: countable-sequence reading

This file starts the Lean formalization of Proposition 6 in the PDF cited
below.
...
Mathematically, the proof is the greedy construction: for a countable sequence
of infinite sets `A n`, choose fresh points `x n, y n ∈ A n` at stage `n`, then
color every `x n` true and every `y n` false.
-/
...
/--
Once the red witnesses and blue witnesses are globally disjoint, the final
...
/--
Target theorem for the countable-sequence reading of Problem #603.

The proof is mathematically straightforward from `greedyPrefix`, but the last
Lean step needs the global disjointness invariant saying that no later blue
point is ever one of the earlier red points, and no later red point is ever one
of the earlier blue points.
-/
